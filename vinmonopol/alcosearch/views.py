...
def info(request):
    if request.method == "POST":
        if request.POST.get("Varetype"):
            ...
            limitBy = request.POST.get("limitID")
            if (len(limitBy) == 0):
                limitBy = "50"
            output = ""
            for i in range(len(thelist)):
                ...
                if i == len(thelist) - 1:
                    ...
                    output += thelist[i]
            Apl = Fullinfo.objects.raw(
                "select ID, Varenummer, Varenavn, Varetype, (Literpris * (100/Alkohol)) as Apl, Alkohol, Pris, Produktutvalg, Vareurl from Fullinfo where " + output.replace("%", "%%") + " order by Apl limit " + limitBy + ";")
            title = ""
        else:
            return render(request, "alcosearch/main.html", {"vin": vin, "alkoholfritt": alkoholfritt, "ol": ol, "champagne": champagne, "brennevin": brennevin, "varetype": varetype})
